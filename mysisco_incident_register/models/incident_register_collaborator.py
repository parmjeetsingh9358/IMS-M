...
class IncidentRegisterCollaborator(models.Model):
    _name = "incident.register.collaborator"
    _description = "Collaborators in incident register shared"
    ...
    _sql_constraints = [
        (
            ...
            "A collaborator cannot be selected more than once in the "
            "register sharing access. Please remove duplicate(s) and try again.",
        ),
    ]
    ...
    def create(self, vals_list):
        collaborator = self.env["incident.register.collaborator"].search([], limit=1)
    ...
    def _toggle_register_sharing_portal_rules(self, active):
        ...
        When the first collaborator is added in the
         model then we need to enable the feature.
        ...
        added to portal user that we do not want to give
        when we know the register sharing is unused.
        :param active: contains boolean value, True to enable
        the register sharing feature, otherwise we disable the feature.
        """
        access_incident_register_sharing_portal = self.env.ref(
            "mysisco_incident_register.access_incident_register_sharing_task_portal"
        ).sudo()
        if access_incident_register_sharing_portal.active != active:
            access_incident_register_sharing_portal.write({"active": active})

        task_portal_ir_rule = self.env.ref(
            "mysisco_incident_register.incident_register_rule_portal"
        ).sudo()
